            ...
            # Domain:         customer
            # Mode:           BOOTSTRAP
            # Generated at:   2026-05-18T20:19:19.853094Z
            # Schema version: 1
            # Target table:   anz-cloud-migration:phi_bronze_customer.src_customers_001_raw
            # Dataplex asset: to-be-registered
            #
            # PII transforms (3 columns):
            #   
AadharIdentifier (PII_AADHAAR)#   TelephoneNumber (PII_MOBILE)#   DateOfBirth (PII_DOB)
            #
...
COLUMN_ORDER = ['AadharIdentifier', 'PartyName', 'TelephoneNumber', 'GeographicAddressLocalityName', 'ElectronicAddress', 'DateOfBirth', 'PartyStatusType', 'PartyStatus', 'PartyIdentifier']

# BigQuery schema with policy tag resource names embedded.
# BQ enforces column-level access based on these tags at query time.
BQ_SCHEMA = {"fields": [{'name': 'AadharIdentifier', 'type': 'STRING', 'mode': 'NULLABLE', 'policyTags': {'names': ['projects/anz-cloud-migration/locations/us-central1/taxonomies/4502003670685444846/policyTags/4887491302710355422']}}, {'name': 'PartyName', 'type': 'STRING', 'mode': 'NULLABLE'}, {'name': 'TelephoneNumber', 'type': 'STRING', 'mode': 'NULLABLE', 'policyTags': {'names': ['projects/anz-cloud-migration/locations/us-central1/taxonomies/4502003670685444846/policyTags/5296288691026736836']}}, {'name': 'GeographicAddressLocalityName', 'type': 'STRING', 'mode': 'NULLABLE'}, {'name': 'ElectronicAddress', 'type': 'STRING', 'mode': 'NULLABLE'}, {'name': 'DateOfBirth', 'type': 'DATE', 'mode': 'NULLABLE', 'policyTags': {'names': ['projects/anz-cloud-migration/locations/us-central1/taxonomies/4502003670685444846/policyTags/4405612200200574405']}}, {'name': 'PartyStatusType', 'type': 'DATE', 'mode': 'NULLABLE'}, {'name': 'PartyStatus', 'type': 'STRING', 'mode': 'NULLABLE'}, {'name': 'PartyIdentifier', 'type': 'STRING', 'mode': 'NULLABLE'}]}

# PII masking rules — applied BEFORE any data reaches BigQuery
MASKING_RULES: Dict[str, str] = {'AadharIdentifier': 'TOKENIZE', 'TelephoneNumber': 'PARTIAL_MASK', 'DateOfBirth': 'YEAR_ONLY'}
    ...
    "autoscaling_algorithm": "THROUGHPUT_BASED",
            ...
            class ParseAndNormaliseFn(beam.DoFn):
                ...
                def process(self, raw):
                    ...
                    try:
                        row = {
        "AadharIdentifier": apply_masking("AadharIdentifier", clean_aadhaar(row.get("AadharIdentifier", ""))),
        "PartyName": str(row.get("PartyName", "") or "").strip() or None,
        "TelephoneNumber": apply_masking("TelephoneNumber", clean_phone(row.get("TelephoneNumber", ""))),
        "GeographicAddressLocalityName": str(row.get("GeographicAddressLocalityName", "") or "").strip() or None,
        ...
        "PartyStatusType": clean_status(row.get("PartyStatusType", "")),
        "PartyStatus": clean_status(row.get("PartyStatus", "")),
        "PartyIdentifier": str(row.get("PartyIdentifier", "") or "").strip() or None,
                        }
